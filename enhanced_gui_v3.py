...
class EnhancedFirewallGUI(QMainWindow):
    ...
    def setup_generator_tab(self):
        ...
        # NEW: App selection mode
        mode_layout = QHBoxLayout()
        self.app_mode_group = QButtonGroup()
        
        self.auto_mode_radio = QRadioButton("🔄 Auto (use all from diagnostics)")
        self.auto_mode_radio.setChecked(True)
        self.auto_mode_radio.toggled.connect(self.on_app_mode_changed)
        self.app_mode_group.addButton(self.auto_mode_radio)
        mode_layout.addWidget(self.auto_mode_radio)
        
        self.manual_mode_radio = QRadioButton("✋ Manual (select below)")
        self.manual_mode_radio.toggled.connect(self.on_app_mode_changed)
        self.app_mode_group.addButton(self.manual_mode_radio)
        mode_layout.addWidget(self.manual_mode_radio)
        
        app_layout.addLayout(mode_layout)
        
        # Filter box (only visible in manual mode)
        ...
        app_layout.addWidget(self.filter_widget)
        ...
        # Quick select buttons (only visible in manual mode)
        ...
        self.quick_select_widget = QWidget()
        self.quick_select_widget.setLayout(btn_layout)
        app_layout.addWidget(self.quick_select_widget)
    ...
    def on_app_mode_changed(self):
        """Handle app selection mode changes (auto vs manual)"""
        is_auto = self.auto_mode_radio.isChecked()
        
        # Show/hide manual selection controls
        self.filter_widget.setVisible(not is_auto)
        ...
        if is_auto:
            ...
            self.status_label.setText("Manual mode: Select apps below")
    ...
    def load_spindump_file(self):
        ...
        if file_path:
            try:
                ...
                # Populate app list from diagnostics
                self.populate_apps_from_diagnostics()
                
                # Update analysis tab
                self.update_process_analysis()
                
                # Enable rule generation
